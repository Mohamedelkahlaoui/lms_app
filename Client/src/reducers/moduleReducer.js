...
export const updateModule = (courseId, moduleId, module) => {
  return async (dispatch) => {
    try {
      const response = await moduleService.updateModule(courseId, moduleId, module);
  ...
  return async (dispatch) => {
    try {
      if (moduleItem instanceof FormData) {
        ...
        const file = moduleItem.get('file'); // Get the File object directly
        if (file) { // Check if a file was actually selected
          const fileURL = await s3Service.uploadFile(
            ...
            file.name,
            file,
            file.type // Pass the File type
          );
          moduleItem.append('url', fileURL);
        } else {
          throw new Error('No file selected for upload.'); // Handle the case where no file is selected
        }
      }

      const response = await moduleService.createModuleItem(courseId, moduleId, moduleItem);
      dispatch({ type: CREATE_MODULE_ITEM, data: response });
      notification.success({
        message: 'Added module item successfully',
      });
    } catch (error) {
      console.error('Error in createModuleItem action:', error); // Log the error for debugging purposes
      notification.error({
        message: error.message || "Couldn't add module item", // More informative error message
      });

      // Reset the loading state in case of error
      dispatch({ type: LOADING_UPLOAD, moduleId: null });
    }
  };
};
...
export const deleteModuleItem = (courseId, moduleId, moduleItem) => {
  return async (dispatch) => {
    try {
      ...
      const response = await moduleService.deleteModuleItem(courseId, moduleId, moduleItem.id);
