...
const Profile = () => {
  ...
  const onsave = () => {
    if (photo) {
      handleUpload(); // Call handleUpload if there is a photo to upload
    } else {
      // If no photo is selected, update the profile without changing the photo
      dispatch(
        editProfile({
          name: name,
          email: email,
  ...
  const onFileChange = (pictures) => { // Updated for react-images-upload
    setPhoto(pictures[0]); // Assuming you only want to upload one image
    setActive(false);
  };

  const handleUpload = async () => {
    if (photo) { // Only upload if a photo is selected
      try {
        const response = await ReactS3.uploadFile(photo, config);
        console.log(response);
        setActive(true);

        // After successful upload, dispatch the action to update the profile
        dispatch(
          editProfile({
            name: name,
            email: email,
            username: userName,
            mobile: mobile,
            // Use the S3 URL from the response for the photo
