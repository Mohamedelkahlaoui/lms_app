...
const createUser = async (req, res) => {
  try {
    ...
    // Assuming you are using multer or similar middleware
    if (req.file) {
      user.image = req.file.buffer;
    } else {
      // Default Image Handling:
      // Option 1: Store default image path
      // user.image = '/path/to/default/image.jpg';

      // Option 2: Load default image buffer and store it
      // const defaultImageBuffer = fs.readFileSync('path/to/default/image.jpg');
      // user.image = defaultImageBuffer;
    }
    user.code = Date.now()
    await user.save()
